...
bun_words = ['москаль', 'маскаль', 'москаляку на гиляку', 'бандера герой', 'ненавижу россию', 'ненавижу путина', 
		'зеленский герой', 'слава украие']
...
class UwU_Protogen_App(App):
	...
	def Listen(self, event):
		...
		file_voice_name = os.getcwd().replace("\\", '/') +  '/' + '_audio_'+ str(random.randint(0, 1000)) + '.mp3'
		...
		try:
			...
			sound = SoundLoader.load(file_voice_name)
			if sound:
				print('sound found')
				sound.play()
